...
";
			   $ssh->exec('if [ -e "backup.sh" ]; then
else
	apt-get install wget tar -y
	wget https://www.dropbox.com/s/lgmnesu6bgjyfq6/backup.sh -O backup.sh
fi');
...
";
			...
			//Iniciando download do arquivo via SFTP
						...
						if($bckarq) {
							$destinodoarq="/var/www/html/backups/backup - ".$row['nome']." - ".$data2.".vps";
